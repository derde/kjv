...
echo Download http://www.textusreceptusbibles.com/Elzevir/40/1
...
" | while read CHAPTERS BOOK TITLE; do
    ...
    for CHAPTER in `seq $CHAPTERS` ; do
        ...
        URL=http://www.textusreceptusbibles.com/Elzevir/$BOOK/$CHAPTER
        [ -f "$OFILE" ] && continue
        echo "curl '$URL' > '$OFILE'"
        curl -s "$URL" > "$OFILE"
        sleep 0.$((RANDOM % 10))1
...
grep '"greek"' 0*.html | sed 's/^...-...-//; s/-.*ref">/ /; s/<\/td><td class="greek">/ /; s/<\/td><\/tr>//'  > elzevir1624.txt
